...
onready var ViewPortCounter = $Viewport
...
func _ready():
	connect("AmmoUpdate",ViewPortCounter,"CountAmmo")
	emit_signal("AmmoUpdate", CurrentAmmo, CurrentCapacity)
	ViewPortCounter.MaxCap = MagazineCapacity
...
func _PrimaryFire(Speed: float = 0.0):
	if AnimPlayer.is_playing():
		...
		if CurrentAmmo == 0:
			...
			#set_rotation_degrees(Vector3(min(_spray.y/2,12),0,0)) #jerks the gun around, doesn't really effect spray aim.
